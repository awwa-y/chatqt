...
public:
	RedisConPool(size_t poolSize, const char* host, int port, const char* pwd)
		...
		for (size_t i = 0; i < poolSize_; ++i) {
			...
			if (context == nullptr) {
				std::cerr << "Redis连接失败: 无法分配Redis上下文，主机: " << host << "，端口: " << port << std::endl;
				fail_count_++;
				continue;
			}

			if (context->err != 0) {
				std::cerr << "Redis连接错误: " << context->errstr << "，主机: " << host << "，端口: " << port << std::endl;
				redisFree(context);
				fail_count_++;
				continue;
			}
			...
			if (reply == nullptr) {
				std::cerr << "Redis认证错误: AUTH命令返回null，连接索引: " << i << std::endl;
				redisFree(context);
				fail_count_++;
				continue;
			}
			...
			if (reply->type == REDIS_REPLY_ERROR) {
				std::cerr << "Redis认证失败: " << reply->str << "，连接索引: " << i << std::endl;
